...
def create_feature_extractor(architecture, input_size, weights=None, strip_size = 32, alpha=0.75):
    """
    # Args
        architecture : str
        input_size : int
    ...
    if architecture == 'Inception3':
        ...
        feature_extractor = MobileNetFeature(input_size, weights, strip_size = strip_size ,alpha=alpha)
...
class MobileNetFeature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size, weights=None, strip_size=32,alpha=0.75):
        import tensorflow as tf
        from tensorflow.keras.models import Model
        ...
        if type(weights) != str or not os.path.exists(weights):
            print("没有本地的 MobileNet 权重文件，将自动下载。| No local MobileNet weight file, will download automatically.")  
            print("您可以手动下载并提供其路径作为权重参数。| You can download it manually and give its path as the weights argument.")
            weights = "imagenet"
        input_image = Input(shape=(input_size[0], input_size[1], 3))
        mobilenet = MobileNet(input_shape=(input_size[0], input_size[1],3),alpha = alpha,depth_multiplier = 1, dropout = 0.001, 
