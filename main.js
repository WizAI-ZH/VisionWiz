...
function launchInstallerAndQuit(installerPath) {
  ...
  const helperScript = [
    ...
    "$cacheDir = Split-Path -Parent $installer",
    "$cleanupPatterns = @('VisionWiz-Setup-*.exe', 'VisionWiz-update-helper-*.log', 'VisionWiz-update-helper-*.ps1')",
    "foreach ($pattern in $cleanupPatterns) {",
    "  Get-ChildItem -LiteralPath $cacheDir -Filter $pattern -File -ErrorAction SilentlyContinue | ForEach-Object {",
    "    if ($_.FullName -ne $helperPath) { Remove-Item -LiteralPath $_.FullName -Force -ErrorAction SilentlyContinue }",
    "  }",
    "}",
